...
if(!dir.exists(refdir))
  stop(paste("Path for reference folder not found:", refdir))
...
if(!dir.exists(slideseqdir))
  stop(paste("Path for Slideseq folder not found:", slideseqdir))
...
if(!dir.exists(bulkdir))
  dir.create(bulkdir)
if(!file.exists(file.path(slideseqdir,config$puckfile)))
  stop(paste("Path for Slideseq file not found:", file.path(slideseqdir,config$puckfile)))
...
if(!file.exists(paste(refdir,config$reffile,sep="/")))
  stop(paste("Path for reference file not found:", paste(refdir,config$reffile,sep="/")))
