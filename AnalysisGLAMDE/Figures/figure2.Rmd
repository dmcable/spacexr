...
# Benchmarking GLAMDE on a simulated spatial transcriptomics dataset
...
library(reshape2)
library(dplyr)
source('~/Documents/MIT/Research/Rafalab/Projects/spacexr/AnalysisGLAMDE/helper_functions/de_simulation_helper.R')
devtools::load_all()
load_all()
set_global_Q_all()
...
### Run RCTD on simulated data (the first step of GLAMDE)
...
### Run GLAMDE on contamination simulation
...
colnames(plot_df) <- c('log_fc_b', 'log_fc_p', 'GLAMDE_b', 'GLAMDE_p', 'dec_b', 'dec_p', 'bulk','sing_b','sing_p', 'sd_GLAMDE_b','sd_GLAMDE_p','sd_dec_b','sd_dec_p','sd_bulk', 'sd_sing_b','sd_sing_p')
graph_df <- plot_df %>% 
  select(log_fc_p, GLAMDE_b, sd_GLAMDE_b) %>% 
  rename(estimate=GLAMDE_b, sd=sd_GLAMDE_b) %>%
  mutate(type = 'GLAMDE_b') %>%
  bind_rows(plot_df %>% 
              select(log_fc_p, GLAMDE_p, sd_GLAMDE_p) %>% 
              rename(estimate=GLAMDE_p, sd=sd_GLAMDE_p) %>%
              mutate(type = 'GLAMDE_p')) %>%
  bind_rows(plot_df %>% 
...
p1 <- ggplot(graph_df[graph_df$type %in% c('GLAMDE_p', 'dec_p', 'bulk', 'sing_p'),], aes(x=log_fc_p,y=estimate,group=type, color = type)) + geom_line() + theme_classic() + geom_abline(slope=1,intercept=0) + geom_errorbar(aes(ymin = estimate - sd*1.96/sqrt(REPLICATES),ymax=estimate+sd*1.96/sqrt(REPLICATES)), width = 0.03) + ylab('Estimated cell type A DE')+ xlab('Differential expression in cell type A') + ggplot2::scale_color_manual("Method",values = c('#0072B2','#009E73','#D55E00', '#EFCB00'), breaks = c('bulk', 'dec_p' ,'GLAMDE_p', 'sing_p'), labels = c('Bulk','Decompose','GLAMDE','Single')) + scale_x_continuous(breaks = unique(graph_df$log_fc_p), labels = round(unique(graph_df$log_fc_p),2))
p2 <- ggplot(graph_df[graph_df$type %in% c('GLAMDE_b', 'dec_b', 'bulk', 'sing_b'),], aes(x=log_fc_p,y=estimate,group=type, color = type)) + geom_line() + theme_classic() + geom_abline(slope=0,intercept=0) + geom_errorbar(aes(ymin = estimate - sd*1.96/sqrt(REPLICATES),ymax=estimate+sd*1.96/sqrt(REPLICATES)), width = 0.03)+ ylab('Estimated cell type B DE')+ xlab('Differential expression in the other cell type')+ ggplot2::scale_color_manual("Method",values = c('#0072B2','#009E73','#D55E00','#EFCB00'), breaks = c('bulk', 'dec_b' ,'GLAMDE_b', 'sing_b'), labels = c('Bulk','Decompose','GLAMDE', 'Single')) + scale_x_continuous(breaks = unique(graph_df$log_fc_p), labels = round(unique(graph_df$log_fc_p),2))
ggarrange(p1,p2, nrow = 2)
```

### Run GLAMDE on cell type proportion simulation
...
colnames(plot_df) <- c('ct_prop','log_fc_b', 'log_fc_p', 'GLAMDE_b', 'GLAMDE_p', 'dec_b', 'dec_p', 'bulk', 'sing_b', 'sing_p', 'sd_GLAMDE_b','sd_GLAMDE_p','sd_dec_b','sd_dec_p','sd_bulk', 'sd_sing_b', 'sd_sing_p')
graph_df <- plot_df %>% 
  select(ct_prop, GLAMDE_b, sd_GLAMDE_b) %>% 
  rename(estimate=GLAMDE_b, sd=sd_GLAMDE_b) %>%
  mutate(type = 'GLAMDE_b') %>%
  bind_rows(plot_df %>% 
              select(ct_prop, GLAMDE_p, sd_GLAMDE_p) %>% 
              rename(estimate=GLAMDE_p, sd=sd_GLAMDE_p) %>%
              mutate(type = 'GLAMDE_p')) %>%
  bind_rows(plot_df %>% 
...
p1 <- ggplot(graph_df[graph_df$type %in% c('GLAMDE_p', 'dec_p', 'bulk', 'sing_p'),], aes(x=ct_prop,y=estimate,group=type, color = type)) + geom_line() + theme_classic() + geom_abline(slope=0,intercept=0) + geom_errorbar(aes(ymin = estimate - sd*1.96/sqrt(REPLICATES),ymax=estimate+sd*1.96/sqrt(REPLICATES)), width = 0.03)  + ylab('Estimated cell type A DE')+ xlab('Difference of mean cell type proportion across regions') + ggplot2::scale_color_manual("Method",values = c('#0072B2','#009E73','#D55E00','#EFCB00'), breaks = c('bulk', 'dec_p' ,'GLAMDE_p', 'sing_p'), labels = c('Bulk','Decompose','GLAMDE','Single')) + scale_x_continuous(breaks = unique(graph_df$ct_prop), labels = round(unique(graph_df$ct_prop),2))
p2 <- ggplot(graph_df[graph_df$type %in% c('GLAMDE_b', 'dec_b', 'bulk', 'sing_b'),], aes(x=ct_prop,y=estimate,group=type, color = type)) + geom_line() + theme_classic() + geom_abline(slope=0,intercept=0) + geom_errorbar(aes(ymin = estimate - sd*1.96/sqrt(REPLICATES),ymax=estimate+sd*1.96/sqrt(REPLICATES)), width = 0.03) + ylab('Estimated cell type B DE')+ xlab('Difference of mean cell type proportion across regions') + ggplot2::scale_color_manual("Method",values = c('#0072B2','#009E73','#D55E00','#EFCB00'), breaks = c('bulk', 'dec_b' ,'GLAMDE_b','sing_b'), labels = c('Bulk','Decompose','GLAMDE','Single'))+ scale_x_continuous(breaks = unique(graph_df$ct_prop), labels = round(unique(graph_df$ct_prop),2))
ggarrange(p1,p2, nrow = 2)
```

### Run GLAMDE on null and non-null DE case
