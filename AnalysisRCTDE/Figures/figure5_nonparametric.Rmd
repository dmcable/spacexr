...
resultsdir <- paste0('../../results/ResultsTumorNonparam','/')
...
```{r} 
p <- list()
resultsdir_par <- paste0('../../results/ResultsTumor','/')
...
for(gene in sig_gene_list) {
  predictions <- predict_DEGLAM(2, gene_fits, gene, new_X)[,1]
...
library(zoo)
library(ggrepel)
sig_lists <- list_pass_thresh[which(p.adjust(all_p_vals[gene_tot >= 5], method = 'BH') < 0.1)]
my_df[sig_lists,]
