...
plot_df$sig <- !is.na(plot_df$label)
p <- ggplot(plot_df, aes(x=log_fc, y = p_val, color = group, alpha = sig)) + geom_point() + theme_classic()  +
  ...
  geom_label_repel(data = plot_df[plot_df$group != 'emt',],aes(label = label),nudge_x = 0,na.rm = TRUE, show.legend = F, size = 3, label.padding = 0.1, max.overlaps = 15)+ xlab('Estimated cell type-specific DE by RCTDE') + ylab('RCTDE p-value') + scale_color_manual("", labels = c('EMT', 'Other'), values = c('#D55E00','black')) + geom_label_repel(data = plot_df[plot_df$group == 'emt',],aes(label = label),nudge_x = 0,na.rm = TRUE, show.legend = F, size = 3, label.padding = 0.1, max.overlaps = 15)+ xlab('Estimated cell type-specific DE by RCTDE') + ylab('RCTDE p-value') + scale_alpha_manual("", labels = c('Not significant', 'Significant'), values = c(0.2,1))
p
