---
title: "Applying GLAMDE to Spatial Transcriptomics Data: Differential Expression with Multiple Experimental Replicates"
author: "Dylan Cable"
...
output:
  ...
  rmarkdown::html_vignette:
    keep_md: yes
vignette: |
  %\VignetteIndexEntry{GLAMDE-replicates} 
...
Generalized Linear Admixture Models for Differential Expression, or GLAMDE, is part of the *spacexr* R package for learning cell type-specific differential expression from spatial transcriptomics data. In this Vignette, we will demonstrate running RCTD and GLAMDE on multiple replicates (slices) of cerebellum Slide-seq toy data. 
...
In order to run GLAMDE, we will for each replicate create a explanatory variable (i.e. covariate) used for predicting differential expression in GLAMDE. In general one should set the explanatory variable to biologically relevant predictors of gene expression such as spatial position. In this case, we will use the explanatory variable two divide each cerebellum replicate into two regions (left and right). In general, one can also create a custom function that will compute the explanatory variable on each `SpatialRNA` replicate. We will pass in a list of the explanatory variables to the `run.GLAMDE.replicates` function.

Here, we also artifically upregulate the expression of the Kcnip4 gene in each dataset (in regions of high explanatory variable) to see whether GLAMDE can detect this differentially expressed gene.
...
```
Now, we are in a position to create a `RCTD.replicates` object, an object that can store multiple `SpatialRNA` objects allowing for joint processing with RCTD and GLAMDE. We pass in `spatial.replicates`, a list of the four `SpatialRNA` objects, `replicate_names`, a list of names for each replicate, and `group_ids`, a list of the group number for each replicate. In this case, all replicates will come from the same group. However, we have the option to assign different replicates to different groups, which will allow for extra variation to be modeled across groups.
...
After running GLAMDE, it is time to examine GLAMDE results. Note that the RCTD and GLAMDE results on individual replicates are stored in `myRCTD.reps@RCTD.reps`, which is a list of `RCTD` objects containing individual GLAMDE results as discussed in other vignettes. For this reason, we now focus on population-level GLAMDE results across the four replicates.
...
Finally, we will save population GLAMDE results in the `savedir` directory:

```{r results_save}
save.GLAMDE.replicates(myRCTD.reps, file.path(savedir,'population'))
```
