...
```{r setupdata}
DropViz <- T
myRCTD <- readRDS('../../myRCTD_cross.rds')
if(DropViz) {
  proportions = myRCTD@internal_vars$proportions
  cell_type_info_unnorm <- myRCTD@cell_type_info$info
}
...
if(DropViz) {
  ...
  common_cell_types <- myRCTD@cell_type_info$info[[2]]
}
```
...
gene_means <- as.matrix(cell_type_info_unnorm[[1]][myRCTD@internal_vars$gene_list_bulk,]) %*% true_proportions
bulk_vec = rowSums(puck@counts);
total_UMI <- sum(puck@nUMI)
true_platform_effect = log(bulk_vec[myRCTDgene_list] / total_UMI,2) -log(gene_means,2)

gene_means_unnorm_pred <- as.matrix(cell_type_info_unnorm[[1]][myRCTD@internal_vars$gene_list_bulk,]) %*% proportions / sum(proportions)
pred_platform_effect = log(bulk_vec[myRCTD@internal_vars$gene_list_bulk] / total_UMI,2) -log(gene_means_unnorm_pred,2)
