#' Runs RCTDE on a \code{\linkS4class{RCTD}} object with a single explanatory variable
#'
...
#' Runs RCTDE on a \code{\linkS4class{RCTD}} object to detect nonparametric smooth gene expression patterns
#'
...
#' Runs RCTDE on a \code{\linkS4class{RCTD}} object for DE across multiple discrete regions
#'
...
#' Runs cell type specific RCTDE on a \code{\linkS4class{RCTD}} object with a general design matrix
#'
...
#' Runs RCTDE on a \code{\linkS4class{RCTD}} object with a general design matrix
#'
...
run.RCTDE.general <- function(myRCTD, X1, X2, barcodes, cell_types, gene_threshold = 5e-5, cell_type_threshold = 125,
  ...
  if(test_genes_sig)
    sig_gene_list <- get_sig_genes(puck, myRCTD, gene_list_tot, cell_types, my_beta, barcodes, nUMI,
  ...
  myRCTD@de_results <- list(gene_fits = gene_fits, sig_gene_list = sig_gene_list)
  return(myRCTD)
}
...
get_sig_genes <- function(puck, myRCTD, gene_list_tot, cell_types, my_beta, barcodes, nUMI,
  ...
  for(cell_type in cell_types) {
    ...
    if(test_mode == 'individual')
      sig_genes <- find_sig_genes_individual(cell_type, cell_types, gene_fits, gene_list_type, X2,
...
test_genes_sig_post <- function(myRCTD, params_to_test = NULL, fdr = .01, p_thresh = 1, log_fc_thresh = 0.4) {
  ...
  sig_gene_list <- get_sig_genes(puck, myRCTD, gene_list_tot, cell_types, my_beta, barcodes, nUMI,
                                 ...
                                 p_thresh = p_thresh, log_fc_thresh = log_fc_thresh)
  myRCTD@de_results$sig_gene_list <- sig_gene_list
  return(myRCTD)
}
...
find_sig_genes_categorical <- function(cell_type, cell_types, gene_fits, gene_list_type, X2, fdr = 0.01,
  ...
  sig_genes <- data.frame(sd_lfc_vec[gene_list_sig], i1_vec[gene_list_sig], i2_vec[gene_list_sig],
