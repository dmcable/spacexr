...
#' Makes quantitative RCTDE plots on RCTD object, after running RCTDE
#'
#' @param myRCTD \code{\linkS4class{RCTD}} object containing \code{de_results}, after running RCTDE
#' @param datadir output directory
#' @export
...
#' Makes spatial gene RCTDE plots (colored continuously) on RCTD object, after running RCTDE
#'
#' @param myRCTD \code{\linkS4class{RCTD}} object containing \code{de_results}, after running RCTDE
#' @param datadir output directory
#' @export
...
#' Makes spatial gene RCTDE plots (colored by two discrete regions) on RCTD object, after running RCTDE
#'
...
#' Makes spatial gene RCTDE plots (colored by discrete regions) on RCTD object, after running RCTDE
#'
#' @param myRCTD \code{\linkS4class{RCTD}} object containing \code{de_results}, after running RCTDE
#' @param datadir output directory
#' @export
...
#' Saves to csv the RCTDE significant gene dataframes after running RCTDE
#'
#' @param myRCTD \code{\linkS4class{RCTD}} object containing \code{de_results}, after running RCTDE
#' @param datadir output directory
#' @export
...
plot_prediction_genes <- function(cell_type, barcodes, my_beta, puck, X2, sig_genes, doublet_mode, datadir, gene_fits) {
  ...
  if(length(gene_list_sig) > 0 & length(barcodes_sing) > 0) {
    for(i in 1:length(gene_list_sig)) {#length(gene_list_sig)
      gene = gene_list_sig[i]
      predictions <- predict_RCTDE(cell_type_ind, gene_fits, gene, X2[barcodes_sing,])[,1]
...
#' Makes a spatial plot of RCTDE fitted gene expression
#'
...
plot_prediction_gene <- function(myRCTD, cell_type, gene) {
  ...
  if(length(barcodes_sing) >= 10) {
    predictions <- predict_RCTDE(cell_type_ind, myRCTD@de_results$gene_fits, gene,
...
predict_RCTDE <- function(cell_type_ind, gene_fits, gene, X2_mat) {
...
get_quant_df <- function(RCTDde, gene_fits, cell_types, cur_cell_types, gene, multi_region = F, prop_thresh = 0.999, param_position = 2) {
  ...
  for(cell_type_ind in 1:length(cell_types)) {
    sigma <- as.numeric(gene_fits$sigma_g[gene])/100
    pred_ct <- predict_RCTDE(cell_type_ind, gene_fits, gene, RCTDde@internal_vars_de$X2[rownames(my_beta),])
